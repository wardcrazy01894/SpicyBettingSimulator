...
| `crypto.subtle` PBKDF2 iterations            | **capped at 100,000 inside workerd** (`OperationError` above it)                                   | Independently confirms §10.1: the briefed 100k+ server-side PBKDF2 is not merely too slow, it is _refused_. Our server KDF is 1,000 iterations. Worker-project tests must use precomputed `dk` vectors and never derive them in-pool. |
...
- **workerd caps PBKDF2 at 100,000 iterations** and throws `OperationError` above
  it — so 210k is not merely too slow server-side, it is _refused_. That is an
  independent confirmation of the whole design, and it has one concrete
  consequence for tests: **the `worker` vitest project can never derive a `dk`
  in-pool.** `tests/worker/setup.ts` exports precomputed hex vectors generated in
  Node and pinned by `tests/unit/kdf-parity.spec.ts`. The server-side
  `SERVER_KDF_ITERATIONS = 1_000` is far under the cap and runs for real in those
  tests.
